...
## Important Issue (2025/12)
**Theorem 3 in the original paper is not rigorous; that is, the differential privacy guarantee is (expected to be) not necessarily $\left(\left( k + \frac{|\mathcal{R}|-1}{2} \cdot l \right) \cdot \epsilon\right)$. We revised Supplements.pdf accordingly. In the revised document, we state that differential privacy guarantee value is not more than $\left(\left( k + \frac{|\mathcal{R}|}{2} \cdot l \right) \cdot \epsilon\right)$. However, the true value should be truly less than that and indeed superior to that achieved by the mechanism with two-sided noise. Moreover, it should be even less than $\left(\left( k + \frac{|\mathcal{R}|-1}{2} \cdot l \right) \cdot \epsilon\right)$ when using an appropriate $(\alpha, \beta)$-admissible distribution. (Indeed, the accuracy results of SPS_(II) and SPS_(IV) in Fig. 1 can be higher than those in the original paper.)**

**Regarding this issue, we will revisit the discussion in our next paper (on this topic) and also propose a new mechanism that outperforms the SPS.**
...
e.g.) For any $k \geq 2$ and $l > 0$, the distribution with density $h(z) \propto \frac{1}{|z|^k + l}$ should be $\left( \frac{l^{1/k}}{2 \cdot (k-1)^{\frac{k-1}{k}}} \cdot \epsilon, \ \frac{1}{2 (k-1)} \cdot \epsilon \right)$- ${\it admissible}$. (Detailed analyses on this distribution is provided in [our paper](https://doi.org/10.1007/978-981-96-9101-2_4) in ACISP 2025.)  
...
Errata:  
・Theorem 6, "For any $\gamma > 1$" → "For any $\gamma \geq 2$".
